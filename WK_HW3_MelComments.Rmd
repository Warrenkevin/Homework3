...
*Dont forget SD! put the SD equation into a function -Mel*
...
```
*pretty :) see the HW solutions to use ggloop() to run them all at once so you don't have to copy and paste. -Mel*
...
*Looking for all quantitative variables here*
...
*Zombies killed and years of education are non-normal distributions, so require different methods. Also, don't forget CI (thats the hard one here). -Mel*
...
W <- NULL
for (i in 1:N) {
     W[[i]] <- sample(Zombies$weight, size = T, replace = FALSE)}
...
W <- NULL
 for (i in 1:N) {
     W[[i]] <- sample(Zombies$zombies_killed, size = T, replace = FALSE)}
...
W <- NULL
 for (i in 1:N) {
     W[[i]] <- sample(Zombies$years_of_education, size = T, replace = FALSE)}
...
*Visualizations? Don't forget to answer all parts of the Qs*